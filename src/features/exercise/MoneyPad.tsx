// Pavé de saisie d'un montant en euros, AVEC la virgule (pour la conversion et
// le rendu de monnaie). La validation se fait dans ExerciseView via parseEuros
// (« 3,5 » et « 3,50 » acceptés). Réinitialisé via `key` à chaque essai.
...
export function MoneyPad({ status, onValidate }: MoneyPadProps) {
  ...
  const press = (k: string) => {
    ...
    else setValue((v) => (v.replace(',', '').length >= 5 ? v : v + k))
  }

  return (
    <div className="flex flex-col items-center gap-3">
      <div className="grid min-h-[56px] min-w-[160px] place-items-center rounded-btn border-[3px] border-hairline bg-cream px-4 text-[30px] font-extrabold text-ink">
